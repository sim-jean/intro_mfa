---
title: "Analyse du métabolisme eosical"
format: html
editor: visual
---
...
if(!(file.exists(here("data","external", "mfa_data.csv")))){ 
  ...
  mfa_data <- get_eurostat(#########,  
    # Utilise la fonction get_eurostat qui nécessite le code du jeu de donnée
                         time_format = "num")
...
## C. Visualisation des indicateurs par pays
...
#' visualizer("France", "Domestic extraction")
  ...
  p = data_%>%
    ##########################             # On choisit les pays avec subset ou filter()
    ##########################             # On garde les tonnes par tête
    ##########################             # Choisit la variable 
    ##########################             # Récupère les données agrégées par 
                                           # classe de matériaux
...
## D. Comparaison des profils pays dans le temps
...
#'                       unit_ = "Tonnes per capita")  
  p = mfa_data_labeled %>%
      ...
      # Conserver uniquement l'indicateur environnemental spécifié
      # Garder les données dans l'unité de mesure indiquée
      # Filtrer pour ne conserver que les matériaux agrégés définis dans summary_classes
...
efficiency_data %>%
  # Déclarer l'environnement ggplot
  # Tracer une courbe
...
growth_gdp = gdp_data_labeled%>%
             # Pays que l'on souhaite conserver
             # Grouping by geo (or another grouping variable)
             # Ensure the data is in time order
             # Compute growth rate in percent
  ungroup()%>%
  distinct()
```
