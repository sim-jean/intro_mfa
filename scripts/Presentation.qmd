---
title: "Workshop d'analyse du Métabolisme Social"
...
::: incremental
-   Comprendre les principes de [reproducibilité]{.bg3} de l'analyse
-   Accéder et préparer les [données Eurostat]{.bg3} sur les flux de matières
-   Calculer les [indicateurs métaboliques]{.bg3} clés
-   Visualiser et comparer les profils métaboliques
-   Analyser les tendances et les schémas

:::

[Allez sur la page du projet : `https://github.com/sim-jean/intro_mfa`]{.fragment}
...
::: incremental
-   Ils permettent de :
    -   Regrouper tout le code au sein d'une [architecture reproductible]{.bg4} i.e. claire & commune
    -   Créer des [environnements]{.bg4} afin d'avoir un code *reproductible*, gérant les dépendances entre projets et dans le temps
    -   Partager rapidement et facilement le code entre différents utilisateurs
    -   Suivre dans le temps via le [contrôle de version]{.bg4}, notamment en utilisant Git (ce sera pour une autre fois)
:::
...
::: incremental
-   On parle de [reproducibilité]{.bg4} à plusieurs niveaux dans l'analyse scientifique :
    -   Reproducibilité [computationelle]{.bg3} : pouvoir répliquer exactement les résultats
    -   Reproducibilité [empirique]{.bg3} : refaire les tests d'une autre manière
    -   Reproducibilité [conceptuelle]{.bg3} : retrouver les mêmes résultats avec de nouvelles données
-   Celle ci est en [crise]{.bg4} : beaucoup de résultats en science ne sont pas réplicables
...
::: incremental
1.  Une [architecture]{.bg4} claire et organisée par [projets]{.bg4}:
    -   Des fichiers séparés pour les données brutes, traitées et les scripts
2.  [Contrôle de version]{.bg4} : on ne verra pas ça ensemble
3.  Management des [dépendances]{.bg4} :
...
Le but est d'avoir une [architecture]{.bg4} claire, pour que vos résultats puissent être reproduits, avec :

::: incremental
-   Les [scripts]{.bg4} : c'est le texte, le code qui sera exécuté
-   Les [données]{.bg4} : brutes ou nettoyées, ou additionelles de sources extérieures
-   Les [résultats]{.bg4}: figures, tableaux etc
-   La [documentation]{.bg4} : les rapports, ou encore un fichier qui s'appelle le `Readme.md`, qui détaille le projet, les instructions d'utilisation etc.
:::
...
::: incremental
-   Clé pour rendre le travail [reproductible]{.bg4} :
...
Ensuite, on va télécharger et installer des [packages]{.bg4} utiles :
...
-   `magrittr`: un package qui permet d'utiliser l'opérateur [pipe]{.bg4} `%>%`, qui permet de connecter les opérations entre elles, simplifiant le processus de $g(h(x))$ à `x %>% h() %>% g()`
...
-   On accède à [Eurostat]{.bg4} grâce au package `eurostat`
...
2.  Ecrivez une fonction pour représenter les indicateurs clés de l'analyse MFA: Domestic extraction, Direct material inputs, domestic consumption, imports and exports
...
::::::: columns
:::: column
[Productivité matérielle ]{.bg4}(Resource Productivity ou RP):

::: incremental
-   Quantité de valeur économique (PIB) générée par unité de matière entrante dans l'économie (DMI).
-   capacité à créer de la valeur à partir des [matières qui entrent dans le système économique]{.bg3}.
-   indicateur d'efficience économique orienté vers la [production.]{.bg3}
:::
::::

:::: column
[Efficacité matérielle]{.bg4} (Material Efficiency ou ME):

::: incremental
-   quantité de valeur économique (PIB) générée par unité de matière effectivement consommée par l'économie (DMC).
-   focalise sur [l'utilisation interne des matières]{.bg3} pour créer de la valeur.
-   exclut les matières qui sont simplement réexportées
-   indicateur d'efficience écologique [orienté vers la consommation.]{.bg3}
:::
::::
...
Lisez l'article de [O. Vidal et al. , *Nature Geosciences*, 2013](https://www.researchgate.net/publication/258514690_Metals_for_a_low-carbon_society)
...
-   [Or, argent, platine et métaux précieux]{.bg3} :
...
::: incremental  
-   [Bauxite et autre Aluminium]{.bg3} :
...
![[Rapport de l'IEA, 2020](https://www.iea.org/reports/the-role-of-critical-minerals-in-clean-energy-transitions)](../images/minerals_clean_energy.png){fig-align="center" width="50%"}

![](../images/legend.jpg){fig-align="center" width="80%"}

-------------

![[Rapport de l'IEA, 2020](https://www.iea.org/reports/the-role-of-critical-minerals-in-clean-energy-transitions)](../images/minerals_ev.png){fig-align="center" width="50%"}
...
![[Rapport de l'IEA, 2020](https://www.iea.org/reports/the-role-of-critical-minerals-in-clean-energy-transitions)](../images/clean_energy_minerals.png){fig-align="center" width="50%"}
